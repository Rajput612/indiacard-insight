...
import { Calculator, CreditCard as CreditCardIcon, Sparkles, Globe, Smartphone, Store, AlertTriangle } from "lucide-react";
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        {ownedCards.length === 0 && (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg flex items-center space-x-4">
            <AlertTriangle className="h-6 w-6 text-yellow-600" />
            <div>
              <p className="text-yellow-800 font-semibold">No Credit Cards Added</p>
              <p className="text-yellow-700 text-sm">
                Please add your credit cards in the "My Credit Cards" tab to use the Purchase Advisor.
              </p>
            </div>
          </div>
        )}

        <div className={`${ownedCards.length === 0 ? 'opacity-50 pointer-events-none' : ''}`}>
